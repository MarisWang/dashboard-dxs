...
import {outboundulesHeader, tableConfig} from "../../../../common/tableHeader";
...
export default class ManageLabel extends React.Component {
    ...
    getTableDatas = () => {
        ...
        setTimeout(() => {
            fetchDatas(`/applications/v1/iptables/?chain=OUTPUT&${keyword}=${this.state.securitygroup_id}`, result => {
                this.setState({
                    isLoading: false,
                    tableDatas: result
                });
    ...
    pushOperationControl = () => {
        ...
        let result = outboundulesHeader.find(item => item.dataIndex === "operation");
        if (!result) outboundulesHeader.push(operation);
    };
    /**删除 delete**/
    handleDelete = key => {
        ...
        return (
            <div>
                ...
                <Table rowKey="id"
                       ...
                       columns={outboundulesHeader}
                       expandedRowRender={record =>
                           <Tabs defaultActiveKey="1">
                               <TabPane tab="详情" key="1">
                                   <ul className="displayflexbetween detailList">
                                       <li><span>创建时间:</span>{record["created_at"]}</li>
                                       <li><span>最近更新者:</span>{record["last_updator"]}</li>
                                       <li><span>更新时间:</span>{record["updated_at"]}</li>
                                       <li><span>规则备注:</span>{record["comment"]}</li>
                                   </ul>
                                   <div className="displayflexstart">
                                       <span>表达式: </span>&nbsp;&nbsp;&nbsp;{record["expression"]}</div>
                               </TabPane>
                           </Tabs>
                       }
                />
