...
import {tableConfig, terminalHeader} from "../../../common/tableHeader";
...
export default class ManageLabel extends React.Component {
    ...
    getTableDatas = () => {
        ...
        setTimeout(() => {
            fetchDatas(`/terminal/v1/terminal/`, result => {
                this.setState({
                    isLoading: false,
                    tableDatas: result
                });
    ...
    pushOperationControl = () => {
        ...
        let result = terminalHeader.find(item => item.dataIndex === "operation");
        if (!result) terminalHeader.push(operation);
    };
    /**删除 delete**/
    handleDelete = key => {
        const tableDatas = [...this.state.tableDatas];
         deleteDatas(`/terminal/v1/terminal/${key}`).then((result) => {
    ...
    onSearchTables = (value) => {
        this.setState({isLoading: true});
        fetchDatas(`/terminal/v1/terminal/?search=${value}`, result => {
            this.setState({
                isLoading: false,
                tableDatas: result,
            });
        ...
        return (
            <div className="wrapper">
                ...
                <Table rowKey="id"
                       ...
                       columns={terminalHeader}
                       expandedRowRender={record =>
                           <Tabs defaultActiveKey="1">
                               <TabPane tab="详情" key="1">
                                   <ul className="displayflexbetween detailList">
                                       <li><span>SSH端口::</span>{record["ssh_port"]}</li>
                                       <li><span>HTTP端口::</span>{record["http_port"]}</li>
